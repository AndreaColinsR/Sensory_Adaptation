...
for f=1:size(ff,1)
    ...
    for i_unit=1:size(Data.unit,2)
        ...
        if ~isnan(x1)
            ...
            corr_LatFR(av_unit)=corr(Lat',prev_FR');
            Lat_itouch(av_unit,:)=[median(Lat(touch_lat==1)) median(Lat(touch_lat==2)) median(Lat(touch_lat==3)) median(Lat(touch_lat>=4))];
...
for im=1:numel(mouse_number)
    ...
    disp(['Mouse ' num2str(im) ' =' num2str(100*sum(intercept_mouse{im}(:,2)<=intercept_mouse{im}(:,1))./size(intercept_mouse{im}(:,1),1)) '%'])
...
for im=1:numel(mouse_number)
    ...
    disp(['Mouse ' num2str(im) ' =' num2str(100*sum(slope_mouse{im}(:,2)<=slope_mouse{im}(:,1))./size(slope_mouse{im}(:,1),1)) '%'])
